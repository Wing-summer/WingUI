...
namespace WingUI
{
	...
	CDPI::CDPI()
	{
		m_nScaleFactor = 0;
		m_nScaleFactorSDA = 0;
		m_Awareness = PROCESS_DPI_AWARENESS::PROCESS_PER_MONITOR_DPI_AWARE;
		SetScale(96);
	}
	...
	int CDPI::GetDPIOfMonitor(HMONITOR hMonitor)
	{
		...
		if (IsWindows8Point1OrGreater()) {
			...
			if (hModule != NULL) {
				...
				if (GetDpiForMonitor != NULL && GetDpiForMonitor(hMonitor, MONITOR_DPI_TYPE::MDT_EFFECTIVE_DPI, &dpix, &dpiy) != S_OK) {
	...
	{
		POINT pt{ 1,1 };	// Get the DPI for the main monitor
	...
	UINT CDPI::GetDPI()
	{
		if (m_Awareness == PROCESS_DPI_AWARENESS::PROCESS_DPI_UNAWARE)
		{
			return 96;
		}

		if (m_Awareness == PROCESS_DPI_AWARENESS::PROCESS_SYSTEM_DPI_AWARE)
		{
